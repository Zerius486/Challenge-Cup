...
- 独立模块测试固件：`MainCode/output/standalone_tests/` 下的 UART、CAN、UDP、力传感器和两类电机工程
...
3. 按 [模块测试说明](MainCode/Projects/ModuleTests/README.md) 依次验证 UART、CAN、UDP、力传感器和两类电机。
...
主工程 Debug/Release 交叉编译、六个独立模块测试固件和 MinGW 主机测试均已通过。实物板、电机、RS485 收发器和机械限位仍需按测试说明完成上板验收；编译通过不等同于带载安全验收。
