...
#define ETH_INPUT_PACKET_BUDGET                  (4U)
...
lan8742_IOCtx_t LAN8720_IOCtx = {ETH_PHY_IO_Init,
                               ETH_PHY_IO_DeInit,
                               ETH_PHY_IO_WriteReg,
                               ETH_PHY_IO_ReadReg,
                               ETH_PHY_IO_GetTick};
...
static err_t low_level_init(struct netif *netif)
{
  ...
  if (LAN8742_RegisterBusIO(&LAN8720, &LAN8720_IOCtx) != LAN8742_STATUS_OK)
  {
    return ERR_IF;
  }

  /* LAN8720A and LAN8742 share the LAN87xx basic/status register layout. */
  if(LAN8742_Init(&LAN8720) != LAN8742_STATUS_OK)
  {
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  if (HAL_ETH_Transmit(&EthHandle, &TxConfig,
...
void ethernetif_input(struct netif *netif)
{
  ...
  uint32_t packets;
    ...
    for (packets = 0U; packets < ETH_INPUT_PACKET_BUDGET; ++packets)
    {
...
void ethernet_link_check_state(struct netif *netif)
{
  ...
  PHYLinkState = LAN8742_GetLinkState(&LAN8720);
  ...
  else if(!netif_is_link_up(netif) && (PHYLinkState > LAN8742_STATUS_LINK_DOWN))
  {
    ...
    if(linkchanged)
    {
      ...
      if (HAL_ETH_SetMACConfig(&EthHandle, &MACConf) == HAL_OK &&
          HAL_ETH_Start(&EthHandle) == HAL_OK)
      {
        netif_set_up(netif);
