...
u8 receive_str[USART6_REC_NUM];     //接收缓存数组,最大USART_REC_LEN个字节 
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance==USART6)//如果是串口1，进行串口1 MSP初始化
	{
		...
		GPIO_Initure.Pin=GPIO_PIN_6;			//PA9
		GPIO_Initure.Mode=GPIO_MODE_AF_PP;		//复用推挽输出
		GPIO_Initure.Pull=GPIO_PULLUP;			//上拉
		GPIO_Initure.Speed=GPIO_SPEED_FAST;		//高速
		GPIO_Initure.Alternate=GPIO_AF8_USART6;	//复用为USART1
		HAL_GPIO_Init(GPIOC,&GPIO_Initure);	   	//初始化PA9

		GPIO_Initure.Pin=GPIO_PIN_7;			//PA10
...
{   
  //UART 初始化设置
	UART6_Handler.Instance=USART6;					    //USART1
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)  //中断执行任务函数  被  中断服务函数调用
{
	...
	if(huart->Instance==USART6)//如果是串口1
	{
		...
		if(rec_data=='S')		  	                         //如果是S，表示是命令信息的起始位
			...
			else if((uart_byte_count>0)&&(uart_byte_count<=USART6_REC_NUM))
				{
