...
├─ MainCode/                  STM32F407 主工程和四个独立测试 ELF
...
- 独立测试固件：`MainCode/output/standalone_tests/` 下的 eRob、RobStride、力传感器和 UDP 工程
...
3. 按 [独立测试说明](MainCode/Projects/ModuleTests/README.md) 依次验证 UDP、力传感器和两类电机。
4. 硬件确认无误后，使用 `MainCode/tools/build_firmware.ps1` 构建和 `flash_firmware.ps1` 烧录主固件。

## 当前验证状态

主工程 Debug/Release 交叉编译、四个独立测试 ELF 和 MinGW 主机测试均已通过。实物板、电机、RS485 收发器和机械限位仍需按测试说明完成上板验收；编译通过不等同于带载安全验收。
