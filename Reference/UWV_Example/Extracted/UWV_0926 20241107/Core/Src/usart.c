...
uint8_t sndBuf3[UART_BUF_SIZE] = {0};
uint8_t rcvBuf3[UART_BUF_SIZE] = {0};
uint8_t sndBuf4[UART_BUF_SIZE] = {0};
uint8_t rcvBuf4[UART_BUF_SIZE] = {0};
uint8_t sndBuf6[UART_BUF_SIZE] = {0};
uint8_t rcvBuf6[UART_BUF_SIZE] = {0};

#ifdef RCVDATA_SEND_BACK
bool sendCompleteSign = TRUE,sendCompleteSign2 = TRUE,sendCompleteSign3 = TRUE,sendCompleteSign6 = TRUE;;
#endif
...
#ifdef RCVDATA_SEND_BACK //将接收的数据发送出去
			if(sendCompleteSign == TRUE)//
			{
				sendCompleteSign = FALSE;
				memset((void *)sndBuf, 0, sizeof(sndBuf));
				memcpy(sndBuf, rcvBuf, len);
				//DMA发送
				UartDmaSend(1, sndBuf, len);
			}
#endif
			break;
    	case 2:
...
#ifdef RCVDATA_SEND_BACK //将接收的数据发送出去
			if(sendCompleteSign2 == TRUE)//
			{
				sendCompleteSign2 = FALSE;
				memset((void *)sndBuf2, 0, sizeof(sndBuf2));
				memcpy(sndBuf2, rcvBuf2, len);
				//DMA发送
				UartDmaSend(2, sndBuf2, len);
			}
#endif
			break;
    	case 3:
...
#ifdef RCVDATA_SEND_BACK //将接收的数据发送出去
			if(sendCompleteSign3 == TRUE)//
			{
				sendCompleteSign3 = FALSE;
				memset((void *)sndBuf3, 0, sizeof(sndBuf3));
				memcpy(sndBuf3, rcvBuf3, len);
				//DMA发送
				UartDmaSend(3, sndBuf3, len);
			}
#endif
			break;
    	case 6:
...
#ifdef RCVDATA_SEND_BACK //将接收的数据发送出去
			if(sendCompleteSign6 == TRUE)//
			{
				sendCompleteSign6 = FALSE;
				memset((void *)sndBuf6, 0, sizeof(sndBuf6));
				memcpy(sndBuf6, rcvBuf6, len);
				//DMA发送
				UartDmaSend(6, sndBuf6, len);
			}
#endif
			break;
...
_Bool StartUartRxDMA(uint8_t uartId)
{
	switch(uartId)
	{
		...
		case 2:
			...
			break;
		case 3:
			UartRxData(&huart3, rcvBuf3, UART_BUF_SIZE);
			break;
		case 6:
